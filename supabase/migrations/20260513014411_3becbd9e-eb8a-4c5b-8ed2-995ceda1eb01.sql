...
BEGIN
  ...
  _normalized_phone_digits := regexp_replace(_normalized_phone, '\D', '', 'g');

  SELECT id INTO _patient_uuid
  FROM public.patients
  WHERE created_by = _doctor_id
    AND regexp_replace(coalesce(phone, ''), '\D', '', 'g') = _normalized_phone_digits
  LIMIT 1;
  ...
  IF _patient_uuid IS NULL THEN
    LOOP
      ...
      SELECT COALESCE(MAX((regexp_replace(patient_id, '\D', '', 'g'))::int), 0) + _attempts
      INTO _next_num
      FROM public.patients
