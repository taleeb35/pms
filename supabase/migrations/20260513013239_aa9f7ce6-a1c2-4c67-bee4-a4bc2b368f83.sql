...
BEGIN
  ...
  -- Reuse existing patient if same phone already exists for this doctor
  _normalized_phone := trim(_phone);
  SELECT id INTO _patient_uuid
  FROM public.patients
  WHERE created_by = _doctor_id
    AND replace(replace(coalesce(phone,''), ' ', ''), '-', '') = replace(replace(_normalized_phone, ' ', ''), '-', '')
  LIMIT 1;

  IF _patient_uuid IS NULL THEN
    -- Retry loop to avoid race condition on patient_id unique constraint
    LOOP
