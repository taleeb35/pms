...
BEGIN
  ...
  SELECT d.id, d.approved
  INTO _doctor_record
  FROM public.doctors d
  WHERE d.id = _doctor_id;

  IF _doctor_record.id IS NULL OR _doctor_record.approved IS NOT TRUE THEN
  ...
  IF FOUND AND _leave.leave_type = 'full_day' THEN
    RETURN jsonb_build_object(
      'available', false,
      'reason', COALESCE(NULLIF(trim(_leave.reason), ''), 'Doctor is on leave for this date'),
      'slots', '[]'::jsonb
    );
  END IF;
  ...
  IF COALESCE(_schedule.is_available, false) IS NOT TRUE THEN
    RETURN jsonb_build_object(
      'available', false,
  ...
  IF _schedule.start_time IS NULL OR _schedule.end_time IS NULL OR _schedule.start_time >= _schedule.end_time THEN
    RETURN jsonb_build_object(
      'available', false,
  ...
  _slot_time := _schedule.start_time;

  WHILE _slot_time < _schedule.end_time LOOP
    IF NOT (
      _schedule.break_start IS NOT NULL
      AND _schedule.break_end IS NOT NULL
      AND _slot_time >= _schedule.break_start
      AND _slot_time < _schedule.break_end
    ) THEN
      IF NOT (
        FOUND AND _leave.leave_type = 'half_day_morning' AND _slot_time < time '12:00'
      ) AND NOT (
        FOUND AND _leave.leave_type = 'half_day_evening' AND _slot_time >= time '12:00'
      ) AND NOT EXISTS (
        SELECT 1
...
BEGIN
  ...
  IF FOUND AND _leave.leave_type = 'full_day' THEN
    RAISE EXCEPTION '%', COALESCE(NULLIF(trim(_leave.reason), ''), 'Doctor is on leave for this date');
  END IF;
  ...
  IF _schedule IS NULL OR COALESCE(_schedule.is_available, false) IS NOT TRUE THEN
    RAISE EXCEPTION 'Doctor is not available on this day';
  END IF;

  IF _schedule.start_time IS NULL OR _schedule.end_time IS NULL OR _appointment_time < _schedule.start_time OR _appointment_time >= _schedule.end_time THEN
    RAISE EXCEPTION 'Appointment time is outside the doctor schedule';
  END IF;

  IF _schedule.break_start IS NOT NULL
     AND _schedule.break_end IS NOT NULL
     AND _appointment_time >= _schedule.break_start
     AND _appointment_time < _schedule.break_end THEN
    RAISE EXCEPTION 'Appointment time is during the doctor break';
  END IF;

  IF FOUND AND _leave.leave_type = 'half_day_morning' AND _appointment_time < time '12:00' THEN
    RAISE EXCEPTION 'Doctor is not available in the morning on this date';
  END IF;

  IF FOUND AND _leave.leave_type = 'half_day_evening' AND _appointment_time >= time '12:00' THEN
