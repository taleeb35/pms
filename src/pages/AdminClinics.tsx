...
const AdminClinics = () => {
  ...
  const handleEndTrialAndActivate = async (clinic: Clinic) => {
    const { error } = await supabase.from("clinics").update({ status: "active", trial_end_date: null }).eq("id", clinic.id);
    if (error) {
      toast.error("Failed to end trial");
      return;
    }
    
    // Update referral partner earnings if clinic was referred
    const { data: clinicData } = await supabase.from("clinics").select("referred_by, requested_doctors, payment_plan").eq("id", clinic.id).single();
    if (clinicData?.referred_by) {
      const { data: partner } = await supabase.from("referral_partners").select("*").eq("referral_code", clinicData.referred_by).eq("status", "active").single();
      if (partner) {
    ...
    toast.success("Trial ended - clinic is now subscribed");
    setSelectedClinic({ ...clinic, status: "active", trial_end_date: null });
    fetchClinics();
  };
